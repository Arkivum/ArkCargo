...
parser = argparse.ArgumentParser(prog='mksynth.py', description='processes the metadata produced by mkcargo Analysis a filesystem and create a cargo file to drive an ingest job.')
# Set consts that we want to know about but the user can not (at this time specify)
parser.set_defaults(inFiles = {'halt' : ['files.queue', 'dirs.queue'], 'warn' : ['error.log', 'debug.log', 'failed'], 'good' : ['added', 'modified', 'unchanged', 'symlink', 'directory', 'config', 'removed'] })
