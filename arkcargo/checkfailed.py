...
if __name__ == '__main__':
    ...
    configPath = os.path.join(currentDir, 'config')
    ...
    if os.path.isdir(os.path.join(currentDir, 'snapshot')):
        failedPath = os.path.join(currentDir, 'snapshot', 'failed')
    else:
        # Just check whether this is an older mkcargo output
        failedPath = os.path.join(currentDir, 'failed')

    if not os.path.isfile(failedPath):
        print "There is no failed file to process"
        exit();

    print "Processing failed files for %s-%s..."%(config['dataset'], config['job'])
    processListofPaths(args.output, snapshotDir, failedPath)
