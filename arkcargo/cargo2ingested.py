...
parser.add_argument('--cargoChecklist', nargs='?', metavar='cargoChecklist to analysis', dest='cargoChecklist', type=str, help='the path to the cargo diff to be analysised.', required=True)
...
if __name__ == u'__main__':
    ...
    if not os.path.isfile(configPath):
        sys.stderr.write('this is not a snapshot metadata directory\n')
        exit();
    ...
    if not os.path.isfile(args.cargoChecklist):
        print u"There is no cargoChecklist file to process"
        exit();

    print u"Processing failed files for %s-%s..."%(config['dataset'], config['job'])
    processCargoChecklist(args.output, snapshotDir, args.cargoChecklist)
